...
################################################
ggplot(knee_data, aes(x = treatment, y = shift, color = treatment))+
  geom_jitter(width = .2, height = 0, size= 3, alpha = .6)+
...
no_eyes <- dplyr::filter(knee_data, treatment  != "eyes")
t.test(shift ~ treatment, data = no_eyes, var.equal = TRUE)


no_knees <- dplyr::filter(knee_data, treatment  != "knee")
t.test(shift ~ treatment, data = no_knees, var.equal = TRUE)

no_control <- dplyr::filter(knee_data, treatment  != "control")
t.test(shift ~ treatment, data = no_control, var.equal = TRUE)
...
knee_all2_devs <- knee_data %>% 
  ...
  dplyr::mutate(total2_dev = (shift      - grand_mean)^2,
                group2_dev = (group_mean - grand_mean)^2,
                error2_dev = (shift      - group_mean)^2) 
